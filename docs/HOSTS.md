...
| CI token permissions | Declared per job (`contents: write`, `id-token: write`, `attestations: write`) | Job token has write access to the repository's units; `permissions:` is ignored | The Forgejo workflow relies on the job token; secrets hold only the signing key |
